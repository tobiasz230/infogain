...
export function createFakeTransaction() {
  ...
  return {
    ...
    createdDate: faker.date.betweens({
      from: subtractMonths(date, transactionsFromRecentMonths - 1),
      to: date,
      count: 1,
    })[0],
    currency,
    price,
    rewardPoints: getRewardPointsByTransaction(
      price,
      rewardPropgram.basePointValue,
      rewardPropgram.pointRanges,
    ),
  };
}
